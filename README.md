...
Process a CSV, TSV, or Excel file directly from the terminal. First, download the .py script to the appropiate directory, and update the pathname to where this script is located. Pathnames for input and output files must also be added if not located in the working directory. **Note: Excel files are preferred to avoid encoding and character errors.**
